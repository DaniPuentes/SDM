...
# This step ensures all required packages are installed and loaded.
# These packages are essential for spatial data handling,
# species distribution modeling, and visualization.

# cran_pkgs <- c(
#   "terra", "sf", "dismo", "maxnet",
#   "geosphere", "scales", "patchwork", "spThin", "tidyterra"
# )
# 
# # Check which packages are missing
# missing_cran <- cran_pkgs[!(cran_pkgs %in% installed.packages()[, "Package"])]
# 
# # Install missing packages (only the ones not yet installed)
...
load_pkgs(c( "tidyverse", "terra", "sf", "dismo", "maxnet","geosphere", "scales", "spThin", "tidyterra"))
...
Obviously, we seem to lack records from the Canadian portion of the range. This presents an extreme version of bias--it appears the species resides there, but we have very little data to model its response in the area lacking records. In this case we would suggest 1) looking for more records; and if that did not help; 2) just interpreting the model output for the US-portion of the range.
...
ggplot() +
  ...
  scale_color_manual(
    name = "Data Type",
    values = c(
      "Random"   = alpha("gray", 0.4),
      "Target"   = alpha("black", 0.5),
      "Presences" = alpha("red", 0.7)
    )
  ) +
  labs(
...
#Yes, MaxEnt models, calculates the presences vs the background, therefore, if we change the background information it would normally change how maxent calculates the habitat suitability. Hence, changing a random environment to a targeted environment will change the calculations and will eventually affect the predictions.We can see that in the Random Background vs Targeted Background maps, where the distribution has completelly changed, to a significant part at least.  
...
#I belive those were desirable effect. For example, we see that maxent suitability with random background pulls suitability to the sites where there is currently sampling areas. When we overcome the sampling area bias by extracting from other backgrounds we can see that we have suitability crossing a political border that most likely could not have a real impact on species distribution, just a sample bias. 
